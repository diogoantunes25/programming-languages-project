...
| CIf of bexp * com * com
| CWhile of bexp * com
| CNonDeterChoice of com * com
| CCondGuard of bexp * com
...
let rec ceval_step st c continuation i =
  (fun zero succ n ->
    ...
    | CAsgn (x, a) ->
      let st' = t_update st x (aeval st a) in Success (st', continuation)
    | CSeq (c1, c2) ->
      (match c1 with
       | CNonDeterChoice (c3, c4) ->
    ...
    | CWhile (b, c0) ->
      if beval st b
      then (match ceval_step st c0 continuation i' with
            | Success s ->
              let (st', cont') = s in ceval_step st' (CWhile (b, c0)) cont' i'
            | x -> x)
      else Success (st, continuation)
    | CNonDeterChoice (c1, c2) ->
      ceval_step st c1 ((st, c2) :: continuation) i'
    | CCondGuard (b, c0) ->
      if beval st b
      then ceval_step st c0 continuation i'
      else (match continuation with
            | [] -> Fail
            | p :: continuation' ->
              let (st', c') = p in ceval_step st' c' continuation' i'))
    i
