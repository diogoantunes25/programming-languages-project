...
| Success of (state * (state * com) list)
| Fail
| OutOfGas

(** val ceval_step :
    state -> com -> (state * com) list -> int -> interpreter_result **)

let rec ceval_step st c continuation i =
  (fun zero succ n ->
      if n=0 then zero () else succ (n-1))
    (fun _ -> OutOfGas)
    (fun i' ->
    ...
    | CAsgn (x, y) -> Success ((t_update st x (aeval st y)), continuation)
    | CSeq (c1, c2) ->
      (match c1 with
       | CChoice (c3, c4) ->
         (match ceval_step st c3 continuation i' with
          | Success s ->
            let (st', cont') = s in
            ceval_step st' c2 ((st, (CSeq (c4, c2))) :: cont') i'
          | x -> x)
       | _ ->
         (match ceval_step st c1 continuation i' with
          | Success s -> let (st', cont') = s in ceval_step st' c2 cont' i'
          | x -> x))
    | CIf (b, c1, c2) ->
      if beval st b
    ...
    | CWhile (b, c1) ->
      if beval st b
      then (match ceval_step st c1 continuation i' with
            | Success s -> let (st', cont') = s in ceval_step st' c cont' i'
            | x -> x)
      else Success (st, continuation)
    | CChoice (c1, c2) ->
      (match ceval_step st c1 continuation i' with
       | Success s ->
         let (st', cont') = s in Success (st', ((st, c2) :: cont'))
       | x -> x)
    | CGuard (b, c1) ->
      if beval st b
      then ceval_step st c1 continuation i'
            ...
            | [] -> Fail
            | p :: cont' -> let (st', c') = p in ceval_step st' c' cont' i'))
    i
          ...
          (match result1 with
           ...
           | NoneE _ ->
             ...
             (match result2 with
              ...
              | NoneE _ ->
                let result3 =
                  match parseSequencedCommand steps' xs with
                  | SomeE x ->
                    let (c1, rest) = x in
                    (match firstExpect ('!'::('!'::[]))
                             (parseSequencedCommand steps') rest with
                ...
                (match result3 with
                 ...
                 | NoneE _ ->
                   let result4 =
                     match firstExpect ('('::[])
                             (parseSequencedCommand steps') xs with
                     | SomeE x ->
                       let (c1, rest) = x in
                       (match firstExpect ('!'::('!'::[]))
                                (parseSequencedCommand steps') rest with
